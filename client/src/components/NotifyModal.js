...
const NotifyModal = () => {
    ...
    const handleDeleteAll = () => {
        ...
        if (window.confirm(`You have ${newArr.length} unread notices? Are you sure you wanna delete all?`)) {
    ...
    return (
        <div style={{minWidth: '300px'}}>
            <div className="d-flex justify-content-between align-items-center px-3">
                ...
                {
                    notify.sound 
                    ? <i onClick={handleSound} className="fas fa-bell text-danger" 
                    style={{fontSize: '1.2rem', cursor: 'pointer'}}/>

                    : <i onClick={handleSound} className="fas fa-bell-slash text-danger"
                    style={{fontSize: '1.2rem', cursor: 'pointer'}}/>
                }
            </div>
            <hr className="mt-0" />
